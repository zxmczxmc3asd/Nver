...
RedlineUI.Theme = {
	Background      = Color3.fromRGB(10, 9, 10),
	Elevated        = Color3.fromRGB(18, 15, 16),
	ElevatedHover   = Color3.fromRGB(25, 20, 21),
	Sidebar         = Color3.fromRGB(8, 7, 8),
	Stroke          = Color3.fromRGB(35, 26, 28),
	StrokeLight     = Color3.fromRGB(55, 35, 38),

	Accent          = Color3.fromRGB(185, 28, 40),
	AccentHover     = Color3.fromRGB(210, 40, 55),
	AccentMuted     = Color3.fromRGB(80, 18, 26),
	AccentGlow      = Color3.fromRGB(255, 60, 75),
	AccentDim       = Color3.fromRGB(120, 22, 32),

	Text            = Color3.fromRGB(240, 234, 234),
	TextDim         = Color3.fromRGB(155, 140, 142),
	TextFaint       = Color3.fromRGB(95, 82, 84),
...
function RedlineUI:Notify(opts)
	...
	local icon = Icons.Create(NOTIF_ICON[kind] or "info", accentColor, 20)
	...
	contentLabel.Position = UDim2.fromOffset(32, 22)
...
function RedlineUI:CreateWindow(opts)
	...
	accentDot.Position = UDim2.fromOffset(16, 25)
	...
	TitleLabel.Position = UDim2.fromOffset(34, 25)
	...
	if subtitle ~= "" then
		...
		SubLabel.Position = UDim2.fromOffset(34 + TitleLabel.TextBounds.X + 10, 26)
    ...
    btn.Parent = TopBar
    newCorner(btn, UDim.new(1, 0))

    local icon = Icons.Create(iconName, iconColor, 15)
    icon.AnchorPoint = Vector2.new(0.5, 0.5)
    icon.Position = UDim2.fromScale(0.5, 0.5)
    icon.Parent = btn

    btn.MouseEnter:Connect(function() 
        tween(btn, { BackgroundTransparency = 0.3, BackgroundColor3 = hoverColor }, 0.1) 
    end)
    btn.MouseLeave:Connect(function() 
        tween(btn, { BackgroundTransparency = 0.8, BackgroundColor3 = bgColor }, 0.1) 
    end)
    return btn
end

	local closeBtn    = makeTopBarButton("x",     12, Color3.fromRGB(185,28,40), Color3.fromRGB(220,50,65), Color3.fromRGB(255,255,255))
	local minimizeBtn = makeTopBarButton("minus", 48, Color3.fromRGB(40,35,38),  Color3.fromRGB(60,50,55),  Color3.fromRGB(200,200,200))
	----------------------------------------------------------------
	-- BODY
	----------------------------------------------------------------
	...
	sidebarBorder.BackgroundColor3 = RedlineUI.Theme.Stroke
	...
	function Window:CreateTab(tabOpts)
		...
		local function sectionContainer()
		...
		function Tab:CreateDropdown(o)
			...
			-- dropdown list (positioned absolutely so it overlaps content below)
			local list = Instance.new("Frame")
			list.BackgroundColor3 = RedlineUI.Theme.Elevated
			list.Position = UDim2.new(0, 0, 1, 4)  -- below the box
			...
			for _, opt in ipairs(options) do
				...
				optBtn.MouseEnter:Connect(function()
					tween(optBtn, { BackgroundTransparency = 0.82 }, 0.1)
			...
			box.MouseButton1Click:Connect(function()
				open = not open
				local targetHeight = open and math.min(#options * 34, 170) or 0
				tween(list, { Size = UDim2.new(1, 0, 0, targetHeight) }, 0.18, Enum.EasingStyle.Quint)
				tween(chevron, { Rotation = open and 180 or 0 }, 0.18)
				tween(boxStroke, { Color = open and RedlineUI.Theme.Accent or RedlineUI.Theme.Stroke }, 0.15)
			end)
		...
		function Tab:CreateColorPicker(o)
			o = o or {}
			local color = o.Default or Color3.fromRGB(185, 28, 40)
